...
tidier_framingham <- readRDS('datasets/tidier_framingham.rds')
library(ggplot2)
```
...
```{r plot-univariate-distributions-NormalExercise, exercise=TRUE, exercise.setup='plot-univariate-distributions-NormalExercise-setup'}
# Examine the age histogram
ggplot(tidier_framingham, aes(x = ___)) +
    ___()
```

```{r plot-univariate-distributions-NormalExercise-hint-2, eval=FALSE}
# Examine the age histogram
ggplot(tidier_framingham, aes(x = participant_age)) +
    geom_histogram()
```

```{r plot-univariate-distributions-NormalExercise-check, eval=FALSE}
"Nice!"
```
...
```{r long-data-and-visualizing-variables-over-time-NormalExercise, exercise=TRUE, exercise.setup='long-data-and-visualizing-variables-over-time-NormalExercise-setup'}
tidier_framingham %>%
    select(
...
```{r long-data-and-visualizing-variables-over-time-NormalExercise-hint-2, eval=FALSE}
tidier_framingham %>%
    select(
        followup_visit_number,
        # Select the three cholesterol-based variables
...
```{r long-data-and-visualizing-variables-over-time-NormalExercise-check, eval=FALSE}
"Great!"
```
...
question("",
  answer("The lipoprotein data was not available at visits 1 and 2.", correct = FALSE, message = "FIXME"),
  answer("Most people smoked zero cigarettes per day.", correct = FALSE, message = "FIXME"),
  answer("The participants' age had a "jagged", uneven distribution.", correct = FALSE, message = "FIXME"),
...
```{r make-discrete-variables-human-readable-NormalExercise, exercise=TRUE, exercise.setup='make-discrete-variables-human-readable-NormalExercise-setup'}
...
```{r make-discrete-variables-human-readable-NormalExercise-hint-2, eval=FALSE}
tidier2_framingham <- tidier_framingham %>% 
    mutate(education = case_when(
      # Use the format: variable == number ~ "string"
...
```{r make-discrete-variables-human-readable-NormalExercise-check, eval=FALSE}
"Excellent!"
```
...
tidier2_framingham <- readRDS('datasets/tidier2_framingham.rds')
library(forcats)
library(dplyr)
...
transformed_framingham <- readRDS('datasets/transformed_framingham.rds')
library(tidyr)
library(dplyr)
library(ggplot2)
```
...
```{r compare-the-different-transformations-NormalExercise, exercise=TRUE, exercise.setup='compare-the-different-transformations-NormalExercise-setup'}
...
```{r compare-the-different-transformations-NormalExercise-hint-2, eval=FALSE}
# Plot a histogram of body mass transforms
...
```{r compare-the-different-transformations-NormalExercise-check, eval=FALSE}
"Amazing!"
```
...
```{r how-does-the-distribution-change-MultipleChoiceExercise-hint-1, eval=FALSE}
...
# TODO: Add the question below in the quotation marks
question("",
  answer("Square root and scale don't change the distribution but do change the unit.", correct = FALSE, message = "Almost. While this is true, it's not the only true answer."),
  answer("Logarithm changes the distribution and unit.", correct = FALSE, message = "Almost. While this is true, it's not the only true answer."),
  answer("Body mass already has a good distribution and has the original unit.", correct = FALSE, message = "Almost. While this is true, it's not the only true answer."),
  answer("Scale can make interpreting easier as 1 unit = 1 standard deviation of the original unit.", correct = FALSE, message = "Almost. While this is true, it's not the only true answer."),
  answer("All of the above.", correct = FALSE, message = "Yes! Which type of and when you might transform really depends on the research question, the data values, and how you will want the results from your analyses to be interpreted. This means you need to carefully think about and have justifications for what you do to the data."),
  allow_retry = TRUE
)
```
